...
class AddDialog(wx.Dialog):
	...
	def InitUI(self):
		...
		dLabel = wx.StaticText(pnl,label="Description:")
		...
		sLabel = wx.StaticText(pnl,label="Sequence:")
		...
		authLabel = wx.StaticText(pnl,label="Authorization:")
	...
	def GetValues(self, e):
		validEntry=True
		self.aSpacer.SetLabel("Label set")
		...
		if(files.numCheck(self.pEntry.GetValue())[1] == False):
		...
		if validEntry:
			print(self.cEntry.GetValue())
			print(self.aEntry.GetValue())
			print(self.dEntry.GetValue())
			print(self.pEntry.GetValue())
			print(self.sEntry.GetValue())
...
class Main_Window(wx.Frame):
	...
	def InitUI(self):
		...
		b3 = wx.Button(panel,label='EDIT CARD')
		b3.SetFont(font)
		hbox4.Add(b3,flag=wx.EXPAND, border=10,proportion=1)
		...
		b4 = wx.Button(panel,label="EXPORT TO DATABASE")
		b4.SetFont(font)
		hbox3.Add(b4,flag=wx.EXPAND)
	...
	def OnDelete(self, e):
		...
		if sel != -1:
			print(self.listbox.GetString(sel).split()[2] + " Deleted")
			self.listbox.Delete(sel)

	def OnAdd(self, e):
		kwargs = {'arg1': self.inf}
	...
	def OnImportCards(self, e):
		...
		for i in self.inf.cardList:
			...
			for __ in range(18 - len(self.inf.cardList[i].card)):
				space = space + " "
			self.listbox.Append("Card #: " + self.inf.cardList[i].card + space + self.inf.cardList[i].description)
	
	def AddCard(self,card):
		print(card)
		space = ""
		for __ in range(18-len(card.card)):
			space = space+" "
		self.listbox.Append("Card #: " + card.card + space + card.description)
